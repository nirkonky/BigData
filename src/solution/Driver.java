	...
	public static void main(String[] args) throws IOException,InterruptedException, ClassNotFoundException {
		...
		while (canopyCentersReader.next(centerReader = new ClusterCenter() , neighbors = new IntWritable())) {			
			...
			if(value_of_vector_divided_by_stocks>=1)
			{
					...
					System.err.println("Added: "+value_of_vector_divided_by_stocks);
			}
			else
		...
		/* validating*/
			System.out.println("After removing Zeros: total centers: "+canopyDivisionOfCentroidsMap.keySet().size());
			for (Entry<ClusterCenter, Integer> entrySet : canopyDivisionOfCentroidsMap.entrySet()) {
				System.out.println("K="+ entrySet.getValue());
			}
		...
		for (Entry<ClusterCenter, Integer> entrySet : canopyDivisionOfCentroidsMap.entrySet())
		{
			System.out.println("Number Of centroids for this ClusterCenter :"+entrySet.getValue());
